...
onMounted(async () => {
    ...
    try {
        const post = await managerStore.fetchPost(postId);
        if (post) {
            localPost.value.title = post.title || '';
            localPost.value.caption = post.main_caption || '';
            localPost.value.status = post.status || 'draft';

            const settings = post.settings || {};
            localPost.value.hashtags = settings.hashtags || [];

            if (settings.platform) {
                activePlatformTab.value = settings.platform;
            }

            if (post.scheduled_at) {
                const dt = new Date(post.scheduled_at);
                localPost.value.scheduled_at = dt.toISOString().split('T')[0];
                localPost.value.scheduled_time = dt.toTimeString().substring(0, 5);
            }

            if (post.platform_posts?.length) {
                for (const pp of post.platform_posts) {
                    if (pp.platform && pp.caption) {
                        localPost.value.platform_captions[pp.platform] = pp.caption;
                    }
                }
            }

            // Load generated image
            const completedImage = post.generated_assets?.find(
                a => a.type === 'image' && a.status === 'completed' && a.url
            );
            if (completedImage) {
                localPost.value.image_url = completedImage.url;
            } else if (post.first_media_url) {
                localPost.value.image_url = post.first_media_url;
            }
        }
    } catch {
...
<template>
    <div class="min-h-full bg-gray-950 p-4 sm:p-6 lg:p-8">
        ...
        <div v-else-if="loadError" class="flex flex-col items-center justify-center py-20 gap-4">
            <p class="text-gray-400 text-sm">{{ t('manager.postEditor.loadError') }}</p>
            <button
                @click="goBack"
                class="px-4 py-2 text-sm font-medium rounded-lg bg-gray-700 text-white hover:bg-gray-600 transition-colors"
            >
                ...
                <div class="space-y-6">
                    ...
                    <div class="rounded-xl bg-gray-900 border border-gray-800 p-4 sm:p-6">
                        ...
                        <div class="rounded-lg bg-gray-800 border border-gray-700 overflow-hidden">
                            ...
                            <!-- Post image -->
                            <div v-if="localPost.image_url" class="mx-3 mb-3">
                                <img
                            ...
                            <div v-else class="mx-3 mb-3 rounded-lg bg-gray-700/50 border border-gray-600 h-40 flex items-center justify-center">
