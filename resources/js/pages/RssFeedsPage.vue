...
// Load articles when switching to articles tab or filters change
watch(activeTab, (tab) => {
    if (tab === 'articles') {
        loadArticles();
    }
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ t('rssFeeds.title') }}</h1>
                    <p class="text-gray-500 mt-1">{{ t('rssFeeds.subtitle') }}</p>
                </div>
                <div class="flex items-center gap-2">
                    <RouterLink
                        to="/rss-feeds/today"
                        class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-orange-700 bg-orange-50 border border-orange-200 rounded-lg hover:bg-orange-100 transition-colors"
                    >
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 7.5h1.5m-1.5 3h1.5m-7.5 3h7.5m-7.5 3h7.5m3-9h3.375c.621 0 1.125.504 1.125 1.125V18a2.25 2.25 0 0 1-2.25 2.25M16.5 7.5V18a2.25 2.25 0 0 0 2.25 2.25M16.5 7.5V4.875c0-.621-.504-1.125-1.125-1.125H4.125C3.504 3.75 3 4.254 3 4.875V18a2.25 2.25 0 0 0 2.25 2.25h13.5M6 7.5h3v3H6v-3Z" />
                        </svg>
                        {{ t('rssFeeds.todayFeed') }}
                    </RouterLink>
            ...
            <div v-if="!loading && rssStore.feeds.length > 0" class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
                    <div class="text-2xl font-bold text-gray-900">{{ rssStore.feeds.length }}</div>
                    <div class="text-xs text-gray-500 mt-0.5">{{ t('rssFeeds.statsFeeds') }}</div>
                </div>
                <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
                    <div class="text-2xl font-bold text-gray-900">{{ rssStore.activeFeedsCount }}</div>
                    <div class="text-xs text-gray-500 mt-0.5">{{ t('rssFeeds.statsActive') }}</div>
                </div>
                <div class="bg-white rounded-xl border border-gray-200 px-4 py-3 col-span-2 sm:col-span-1">
                    <div class="text-2xl font-bold text-gray-900">{{ rssStore.totalArticlesCount }}</div>
                ...
                <nav class="-mb-px flex space-x-8">
                    ...
                    >
                        ...
                        <span
                            ...
                            :class="activeTab === 'feeds' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'"
                        >
                    ...
                    >
                        ...
                        <span
                            ...
                            class="ml-2 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
                            :class="activeTab === 'articles' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'"
                        >
            ...
            <template v-else-if="activeTab === 'feeds'">
                ...
                <div v-if="rssStore.feeds.length === 0" class="text-center py-20">
                    <div class="mx-auto w-20 h-20 rounded-2xl bg-gradient-to-br from-orange-50 to-amber-100 flex items-center justify-center mb-6">
                        <svg class="w-10 h-10 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12.75 19.5v-.75a7.5 7.5 0 0 0-7.5-7.5H4.5m0-6.75h.75c7.87 0 14.25 6.38 14.25 14.25v.75M6 18.75a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                        </svg>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-900">{{ t('rssFeeds.noFeeds') }}</h3>
                    <p class="mt-2 text-sm text-gray-500 max-w-sm mx-auto">{{ t('rssFeeds.noFeedsDescription') }}</p>
                    <div class="mt-6">
                        <Button @click="showAddModal = true">
                            <svg class="w-4 h-4 mr-1.5 -ml-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                ...
                <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    <div
                        ...
                        :key="feed.id"
                        class="relative bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg hover:border-gray-300 transition-all duration-200 group"
                    >
                        <!-- Status color bar -->
                        <div
                            class="h-1"
                        ...
                        <div class="p-5">
                            ...
                            <div class="flex items-start justify-between gap-2 mb-2">
                                <div class="flex items-center gap-2.5 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-gray-400 truncate" :title="feed.url">
                                ...
                                <span
                                    ...
                                    :class="statusColors[feed.status_color] || 'bg-gray-100 text-gray-700'"
                                >
                                    {{ feed.status_label }}
                                </span>
                            </div>
                            ...
                            <!-- Error message -->
                            <div v-if="feed.last_error" class="mt-2 rounded-md bg-red-50 p-2">
                                <p class="text-xs text-red-600 line-clamp-2">{{ feed.last_error }}</p>
                            </div>
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                                ...
                                <span v-if="feed.last_fetched_at" class="text-xs text-gray-400" :title="new Date(feed.last_fetched_at).toLocaleString()">
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-100 flex items-center gap-1">
                                <button
                                    ...
                                    class="flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors disabled:opacity-50"
                                ...
                                <button
                                    ...
                                    class="flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-gray-500 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition-colors"
                                >
                                ...
                                <button
                                    ...
                                    class="ml-auto p-1.5 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all"
                                    :title="t('rssFeeds.deleteFeed')"
                                >
                    ...
                    <button
                        ...
                        class="rounded-xl border-2 border-dashed border-gray-200 hover:border-orange-300 hover:bg-orange-50/50 transition-all duration-200 min-h-[200px] flex flex-col items-center justify-center gap-2 text-gray-400 hover:text-orange-500 group"
                    >
                        <div class="w-10 h-10 rounded-xl bg-gray-100 group-hover:bg-orange-100 flex items-center justify-center transition-colors">
            ...
            <template v-else-if="activeTab === 'articles'">
                ...
                <div class="flex flex-col sm:flex-row gap-3 mb-6">
                    ...
                    <select
                        ...
                        class="rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                    >
                ...
                <div v-else-if="rssStore.articles.length === 0" class="text-center py-20">
                    <div class="mx-auto w-20 h-20 rounded-2xl bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center mb-6">
                        <svg class="w-10 h-10 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                        </svg>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-900">{{ t('rssFeeds.noArticles') }}</h3>
                ...
                <div v-else class="space-y-3">
                    <a
                        ...
                        class="block bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md hover:border-gray-300 transition-all duration-200 group"
                    >
                        <div class="flex">
                            <!-- Image -->
                            ...
                            <div class="flex-1 min-w-0 p-4">
                                ...
                                <div class="flex items-center gap-2 mb-1.5">
                                    ...
                                    <span v-if="article.author" class="text-xs text-gray-400">
                                        · {{ article.author }}
                                    </span>
                                </div>

                                <!-- Title -->
                                <h3 class="text-sm font-semibold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-2 mb-1">
                                    {{ article.title }}
                                    <svg class="inline w-3 h-3 ml-0.5 text-gray-300 group-hover:text-blue-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                                    </svg>
                                </h3>

                                <!-- Description -->
                                <p v-if="article.description" class="text-xs text-gray-500 line-clamp-2 leading-relaxed">
                                    {{ article.description }}
                                </p>
                    ...
                    <div v-if="totalPages > 1" class="flex items-center justify-center gap-2 pt-6">
                        <button
                            ...
                            class="px-3.5 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        >
                            {{ t('rssFeeds.prevPage') }}
                        </button>
