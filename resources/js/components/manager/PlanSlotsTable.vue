...
const isSlotGenerating = (slot) => {
    return slot.status === 'generating' || generatingSlots.value.has(slot.id);
};

const handleGenerateSlot = (slotId) => {
    generatingSlots.value.add(slotId);
    emit('generate-slot', slotId);
};
                        ...
                        <tr class="hover:bg-gray-800/30 transition-colors border-b border-gray-800/50" :class="{ 'bg-orange-500/5': selectedSlots.has(slot.id) }">
                            ...
                            <td class="px-3 py-2.5 align-top" @click.stop="startEditing(slot.id, 'topic', slot.topic)">
                                <div class="flex items-start gap-2">
                                    <img
                                        ...
                                        :src="slot.image_url"
                                        :alt="slot.topic"
                                        class="w-10 h-10 rounded object-cover shrink-0 border border-gray-700"
                                    />
                                    <div class="min-w-0">
                                        <textarea
                            ...
                            <td class="px-3 py-2.5">
                                <div class="flex items-center gap-1">
                                    <!-- Generate button -->
                                    <button
                                        v-if="slot.status === 'planned' && !isSlotGenerating(slot)"
                                        @click.stop="handleGenerateSlot(slot.id)"
                                        class="p-1 rounded text-gray-600 hover:text-emerald-400 hover:bg-emerald-400/10 transition-colors"
                                        :title="t('manager.contentList.generateSingle')"
                                    >
                                    ...
                                    <!-- Generating spinner -->
                                    <div v-else-if="isSlotGenerating(slot)" class="p-1">
                                        <svg class="w-4 h-4 text-purple-400 animate-spin" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                                        </svg>
                                    </div>
                                    <!-- Preview button (content_ready) -->
                                    <button
                                        v-if="slot.social_post_id && !isSlotGenerating(slot)"
                                        @click.stop="emit('preview-slot', slot.social_post_id)"
                                        class="p-1 rounded text-gray-600 hover:text-blue-400 hover:bg-blue-400/10 transition-colors"
                                        :title="t('manager.contentList.previewGenerated')"
                                    >
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                                    ...
                                    <button
                                        ...
                                        class="p-1 rounded text-gray-600 hover:text-red-400 hover:bg-red-400/10 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                                    >
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
            ...
            <template v-for="(slot, index) in sortedSlots" :key="slot.id">
                <div class="p-4 space-y-3" :class="{ 'bg-orange-500/5': selectedSlots.has(slot.id) }">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <button
                                v-if="slot.status === 'planned' && !isSlotGenerating(slot)"
                                @click.stop="handleGenerateSlot(slot.id)"
                                class="p-1.5 rounded text-gray-600 hover:text-emerald-400 hover:bg-emerald-400/10 transition-colors"
                                :title="t('manager.contentList.generateSingle')"
                            >
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 0 0 2.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456Z" />
                            ...
                            <!-- Generating spinner (mobile) -->
                            <div v-else-if="isSlotGenerating(slot)" class="p-1.5">
                                <svg class="w-4 h-4 text-purple-400 animate-spin" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                                </svg>
                            ...
                            <button
                                v-if="slot.social_post_id && !isSlotGenerating(slot)"
                                @click.stop="emit('preview-slot', slot.social_post_id)"
                                class="p-1.5 rounded text-gray-600 hover:text-blue-400 hover:bg-blue-400/10 transition-colors"
                                :title="t('manager.contentList.previewGenerated')"
                            >
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                            ...
                            <button
                                @click.stop="handleDelete(slot.id)"
                                :disabled="isSlotGenerating(slot)"
                                class="p-1.5 rounded text-gray-600 hover:text-red-400 hover:bg-red-400/10 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                            >
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                </svg>
                    ...
                    <div class="space-y-2">
                        ...
                        <!-- Image (mobile) -->
                        <div v-if="slot.image_url" class="flex items-center gap-3">
                            <span class="text-[10px] font-semibold text-gray-500 uppercase tracking-wider w-16 shrink-0"></span>
                            <img
                                :src="slot.image_url"
                                :alt="slot.topic"
                                class="w-full max-w-[200px] h-auto rounded-lg border border-gray-700 object-cover"
                            />
                        </div>
