...
const roles = [
    { id: 'content_creator', icon: 'pencil' },
    { id: 'influencer', icon: 'star' },
    { id: 'company', icon: 'building' },
    { id: 'agency', icon: 'megaphone' },
    { id: 'developer', icon: 'code' },
    { id: 'student', icon: 'academic' },
    { id: 'other', icon: 'dots' },
];
...
<template>
    <div>
        <h2 class="text-2xl font-bold text-white mb-2">{{ t('onboarding.role.title') }}</h2>
        <p class="text-gray-400 mb-8">{{ t('onboarding.role.subtitle') }}</p>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <button
                ...
                class="flex flex-col items-center gap-3 p-5 rounded-xl border-2 transition-all duration-200 cursor-pointer"
            ...
            >
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" :d="iconPaths[role.icon]" />
                </svg>
                <span class="text-sm font-medium text-center">{{ t(`onboarding.role.options.${role.id}`) }}</span>
            </button>
        </div>
    </div>
</template>
